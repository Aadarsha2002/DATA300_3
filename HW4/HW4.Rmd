...
# export as csv
write.csv(player_info_dataframe, "HW4/player_info_dataframe.csv", row.names = FALSE)
```

```{r clean_data}
# duplicate the data frame
player_info_dataframe2 <- read_csv("HW4/player_info_dataframe.csv")
# export as csv file "player_info3.csv"
write.csv(player_info_dataframe2, file = "HW4/player_info3.csv")

# loop through each row in the dataframe
for (i in 1:seq_len(nrow(player_info_dataframe2))) {
...
for (i in 1:seq_len(nrow(player_info_dataframe2))) {
  ...
  player_info_dataframe2$age[i] <-
    round(as.numeric(
      difftime(
        Sys.Date(),
        as.Date(player_info_dataframe2$born_date[i], format = "%Y-%B %d"),
        units = "auto"
      )
    ) / 365, 3)
