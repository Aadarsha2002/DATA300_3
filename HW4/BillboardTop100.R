...
artistsData$Artist <- sub(
    pattern = "\n\t\n\t",
    replacement = "",
    x = artistsData$Artist,
    fixed = TRUE
)
artistsData$Artist <- sub(
    pattern = "\n",
...
songTitlesData$Title <- gsub(
    pattern = "\n",
    replacement = "",
    x = songTitlesData$Title,
    fixed = TRUE
)
songTitlesData$Title <- gsub(
    pattern = "\t",
...
temp$Last_Week_Rating <- gsub(
    pattern = "\n",
    replacement = "",
    x = temp$Last_Week_Rating,
    fixed = TRUE
)
temp$Last_Week_Rating <- gsub(
    pattern = "\t",
...
#
#
BillboardTop100 <- data.frame(
    This_Week = 1:100,
    Title = songTitlesData$Title,
    Artist = artistsData$Artist,
    Last_Week = lastWeeksRatingData$Last_Week_Rating,
    Peak_Position = peakPosData$Last_Week_Rating,
    Weeks_On_Chart = weeksOnChartData$Last_Week_Rating
)

# Save this dataframe into the csv file
write.csv(BillboardTop100, file="HW4/HW4.csv", row.names = FALSE)
